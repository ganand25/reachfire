...
export function CoastClient(): React.JSX.Element {
  ...
  const setCurrentSavings = (v: number): void =>
    setInputs((prev) => ({ ...prev, currentSavings: v }));
  const setMonthlySavings = (v: number): void =>
    setInputs((prev) => ({ ...prev, monthlySavings: v }));
  const setTargetRetirementAge = (v: number): void =>
    setInputs((prev) => ({ ...prev, targetRetirementAge: v }));
  ...
  const chartData = useMemo(() => {
    ...
    for (let age = currentAge; age <= targetRetirementAge + 5; age++) {
      ...
      const portfolioAtCoast = projectedPortfolio(
        currentSavings,
        monthlySavings,
        returnRate,
        yearsSaving
      ...
      const coastTarget = coastFireNumber(
        annualExpenses,
        age,
        targetRetirementAge,
        0.04,
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            Find the age when you can stop contributing to investments and let compound growth alone
            carry you to your FIRE number.
          </p>
          <ExportBar
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <ResponsiveContainer width="100%" height={260}>
                <AreaChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  ...
                  />
                  <ReferenceLine
                    x={Math.round(analysis.coastAge)}
            ...
            {analysis.coastAge < 59.5 && (
              <div className="rounded-xl border border-border bg-card p-5">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  You&apos;ll retire at {Math.round(analysis.coastAge)}, before the 59.5
                  penalty-free access age. You need accessible (non-retirement) funds to cover the
                  gap.
                </p>
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-xs text-muted-foreground">
                    Covers {(59.5 - Math.min(analysis.coastAge, 59.5)).toFixed(1)} gap years in
                    taxable accounts, brokerage, or cash
                  </div>
                </div>
              </div>
            )}
