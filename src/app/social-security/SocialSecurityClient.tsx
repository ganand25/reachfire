...
export function SocialSecurityClient(): React.JSX.Element {
  ...
  const setAvgMonthlyEarnings = (v: number): void =>
    setInputs((prev) => ({ ...prev, avgMonthlyEarnings: v }));
  const setAnnualExpenses = (v: number): void =>
    setInputs((prev) => ({ ...prev, annualExpenses: v }));
  const setSpouseEarnings = (v: number): void =>
    setInputs((prev) => ({ ...prev, spouseEarnings: v }));
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                ...
                <div>
                  ...
                  <div className="font-bold text-emerald-500">
                    {formatMoney(fireImpact.portfolioSavings)}
                  </div>
                </div>
                <div className="col-span-2">
                  <div className="text-muted-foreground">Reduced FIRE number</div>
                  <div className="font-bold text-lg">
                    {formatMoney(fireImpact.reducedFireNumber)}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <StatCard
                label="Benefit at 62"
                value={result.monthlyAt62}
                format="currency"
                subtitle="/month"
              />
              <StatCard
                label="Benefit at FRA"
                value={result.monthlyAtFra}
                format="currency"
                subtitle="/month"
              />
              <StatCard
                    ...
                    {result.scenarios.map((s) => (
                      <tr
                        ...
                        className={cn(
                          ...
                          highlight(s.claimAge) ? 'bg-primary/10' : 'hover:bg-accent/20'
                        )}
                      >
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Where the lines cross = breakeven point. Claiming later wins if you live past your
                breakeven age.
              </p>
            ...
            {hasSpouse && result.spouseBenefit !== null && (
              <div className="rounded-xl border border-border bg-card p-5">
                ...
                <p className="text-xs text-muted-foreground mt-3">
                  Spouse benefit is the higher of their own earned benefit or 50% of your PIA at
                  your FRA.
                </p>
              </div>
            )}
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs text-muted-foreground">
                {[
                  {
                    title: 'Earliest claiming age',
                    body: 'Age 62, but with up to 30% permanent reduction from your FRA benefit.',
                  },
                  {
                    title: 'Full Retirement Age (FRA)',
                    body: 'Age 67 for anyone born 1960 or later. No reduction, no bonus.',
                  },
                  {
                    title: 'Delayed credits',
                    body: '+8% per year from FRA to 70. Claiming at 70 gives you 124% of your FRA benefit.',
                  },
                  {
                    title: 'Spousal benefit',
                    body: "A non-working spouse can claim up to 50% of the working spouse's PIA.",
                  },
                  {
                    title: 'Survivor benefit',
                    body: "A widow/widower receives up to 100% of the deceased spouse's benefit.",
                  },
                  {
                    title: 'FIRE strategy',
                    body: 'Many early retirees bridge with portfolio until 70 to lock in maximum SS — reducing lifetime portfolio risk.',
                  },
                ].map((item) => (
