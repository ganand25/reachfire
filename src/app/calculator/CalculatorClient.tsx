...
import { ChevronDown, ChevronUp, Sparkles, Play, Shield } from "lucide-react";
...
import { AIInsightsPanel } from "@/components/AIInsightCard";
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                {aiInsights.length === 0 && (
                  <button
                    ...
                    disabled={aiLoading}
                    className="flex items-center gap-1.5 text-sm px-3 py-1.5 rounded-lg border border-ember text-primary hover:bg-primary/10 transition-colors disabled:opacity-60"
                  >
              ...
              {aiLoading ? (
                <AIInsightsPanel insights={[]} isLoading={true} />
              ) : visibleInsights.length > 0 ? (
                <AIInsightsPanel insights={visibleInsights} onDismiss={handleDismissInsight} />
              ) : !aiLoading && aiInsights.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center">
                  <Sparkles className="w-10 h-10 text-muted-foreground/30 mb-3" />
                  <p className="text-sm text-muted-foreground max-w-sm">
                    Get personalized AI-powered analysis of your FIRE plan — specific insights based on your numbers
                  </p>
                  <div className="flex items-center gap-1.5 text-xs text-emerald-400 mt-3">
                    <Shield className="w-3 h-3" />
                    Your data is only sent to Anthropic&apos;s API for analysis
                  </div>
                </div>
              ) : null}

              {aiInsights.length > 0 && visibleInsights.length === 0 && (
                <div className="text-center py-4">
                  <p className="text-sm text-muted-foreground">All insights dismissed.</p>
                  <button
                    onClick={() => {
