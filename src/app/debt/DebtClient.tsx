...
export function DebtClient(): React.JSX.Element {
  ...
  return (
    <PageEnter>
      ...
      <div className="mb-8">
        <h1 className="font-display text-3xl sm:text-4xl font-bold mb-2">Debt Payoff Accelerator</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border bg-card p-5">
              ...
              {debts.map((debt) => (
                <div key={debt.id} className="p-3 rounded-lg bg-secondary/50 space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    />
                    <button onClick={() => removeDebt(debt.id)} className="text-muted-foreground hover:text-destructive">
            ...
            <div className="mt-4 pt-4 border-t border-border/50 space-y-1 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-bold">{(summary.weightedAverageRate * 100).toFixed(1)}%</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">Min payments</span>
                <span className="font-bold">${summary.totalMinimumPayment.toLocaleString()}/mo</span>
              </div>
            </div>
          </div>
              ...
              {(["avalanche", "snowball"] as const).map((s) => (
                ...
                >
                  ...
                  <div className="text-muted-foreground mt-0.5">
                    {s === "avalanche" ? "Highest interest first — saves the most money" : "Lowest balance first — fastest psychological wins"}
                  </div>
                </button>
              ))}
            </div>
            ...
            {avalanche && snowball && (
              <div className="mt-3 pt-3 border-t border-border/50 text-xs">
                ...
                <div className="font-bold text-emerald-400">
                  {formatMoney(snowball.totalInterestPaid - avalanche.totalInterestPaid)} in interest
                </div>
          ...
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            ...
            <SliderInput label="Investment Return" value={investmentReturn} min={0.03} max={0.12} step={0.005} onChange={setInvestmentReturn} format="percent" />
            <SliderInput label="Investment Years" value={investmentYears} min={5} max={40} onChange={setInvestmentYears} format="years" />
          </div>
        </div>
            ...
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <StatCard
                  ...
                  glow
                />
                <StatCard label="Interest Paid" value={selected.totalInterestPaid} format="currency" subtitle="Total cost of debt" />
                {redirectResult && (
                  <StatCard
              ...
              <div className="rounded-xl border border-border bg-card p-5">
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <AreaChart data={chartData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="year" tick={{ fontSize: 11 }} tickLine={false} label={{ value: "Year", position: "insideBottom", offset: -2 }} />
                    ...
                    <Area type="monotone" dataKey="balance" stroke="oklch(0.52 0.18 250)" fill="url(#debtGrad)" strokeWidth={2.5} name="Total Debt" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
              ...
              {redirectResult && (
                <div className="rounded-xl border border-border bg-card p-5">
                  ...
                  <p className="text-xs text-muted-foreground mb-4">
                    Once debt is gone, you redirect ${redirectResult.monthlyRedirectAmount.toFixed(0)}/mo to investments. Here&apos;s what that builds:
                  </p>
                  <div className="text-3xl font-bold gradient-ember-text tabular-nums">
                    {formatMoney(redirectResult.projectedValue)}
                  </div>
                  <p className="text-sm text-muted-foreground mt-1">
                    invested over {investmentYears} years at {(investmentReturn * 100).toFixed(1)}% return
                  </p>
                </div>
              )}
            </>
