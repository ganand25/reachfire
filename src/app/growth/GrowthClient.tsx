...
export function GrowthClient(): React.JSX.Element {
  ...
  const setInitialInvestment = (v: number): void =>
    setInputs((prev) => ({ ...prev, initialInvestment: v }));
  const setMonthlyContribution = (v: number): void =>
    setInputs((prev) => ({ ...prev, monthlyContribution: v }));
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            Compound interest is the 8th wonder of the world — and the hockey stick moment is when
            the market works harder for you than you do.
          </p>
          <ExportBar
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          ...
          <div className="rounded-xl border border-border bg-card p-4 flex flex-col justify-center">
            ...
            <p className="text-xs text-muted-foreground">
              Rule of 72 at {(returnRate * 100).toFixed(1)}%
            </p>
          </div>
        </div>
