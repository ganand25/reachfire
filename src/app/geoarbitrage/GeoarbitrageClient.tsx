...
export function GeoarbitrageClient(): React.JSX.Element {
  ...
  return (
    <PageEnter>
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-10">
        <div className="mb-8">
          <h1 className="font-display text-3xl sm:text-4xl font-bold mb-2">
            Geoarbitrage Explorer
          </h1>
          <p className="text-muted-foreground max-w-2xl">
            Your FIRE number in San Francisco becomes a fraction of that in Lisbon or Chiang Mai.
            See exactly how much you save by relocating.
          </p>
          <ExportBar
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center h-9 px-3 rounded-lg border border-border bg-background/50">
                <span className="text-sm font-bold gradient-ember-text">
                  {formatMoney(baseFireNumber)}
                </span>
              </div>
              <p className="text-xs text-muted-foreground">
                in {baseCityData?.name ?? 'home city'}
              </p>
            </div>
          </div>
        </div>
              ...
              >
                ...
                {savings > 0 ? (
                  <div className="text-xs text-emerald-400 mb-3">
                    Save {formatMoney(savings)} ({savingsPct.toFixed(0)}%) vs{' '}
                    {baseCityData?.name?.split(',')[0]}
                  </div>
                ) : (
      ...
      <NextSteps currentPath="/geoarbitrage" />
      <DisclaimerBanner />
    </PageEnter>
  );
}
