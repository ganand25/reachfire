...
const METRIC_THRESHOLDS = {
  ...
  cashOnCash: { good: 0.05, great: 0.1 },
...
export function RealEstateClient(): React.JSX.Element {
  ...
  const setPurchasePrice = (v: number): void =>
    setInputs((prev) => ({ ...prev, purchasePrice: v }));
  const setDownPaymentPct = (v: number): void =>
    setInputs((prev) => ({ ...prev, downPaymentPct: v }));
  ...
  const setMonthlyExpenses = (v: number): void =>
    setInputs((prev) => ({ ...prev, monthlyExpenses: v }));
  const setAnnualAppreciation = (v: number): void =>
    setInputs((prev) => ({ ...prev, annualAppreciation: v }));
  const setCurrentPortfolio = (v: number): void =>
    setInputs((prev) => ({ ...prev, currentPortfolio: v }));
  const setMonthlySavings = (v: number): void =>
    setInputs((prev) => ({ ...prev, monthlySavings: v }));
  const setAnnualExpenses = (v: number): void =>
    setInputs((prev) => ({ ...prev, annualExpenses: v }));
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    ...
                    <div className="font-bold text-emerald-500">
                      {formatMoney(fireImpact.fireNumberWith)}
                    </div>
                  </div>
                </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="rounded-xl border border-border bg-card p-4 text-center">
                <div
                  className={cn(
                    ...
                    ratingColor(metrics.capRate, METRIC_THRESHOLDS.capRate)
                  )}
                >
              ...
              <div className="rounded-xl border border-border bg-card p-4 text-center">
                <div
                  className={cn(
                    'text-2xl font-bold tabular-nums',
                    ratingColor(metrics.cashOnCashReturn, METRIC_THRESHOLDS.cashOnCash)
                  )}
                >
              ...
              <div className="rounded-xl border border-border bg-card p-4 text-center">
                <div
                  className={cn(
                    'text-2xl font-bold tabular-nums',
                    ratingColor(metrics.grossRentMultiplier, METRIC_THRESHOLDS.grm, true)
                  )}
                >
              ...
              <div className="rounded-xl border border-border bg-card p-4 text-center">
                <div
                  className={cn(
                    'text-2xl font-bold tabular-nums',
                    metrics.totalROI * 100 >= 10 ? 'text-emerald-500' : 'text-amber-500'
                  )}
                >
              ...
              <div className="space-y-2 text-sm">
                {[
                  ...
                  <div
                    key={row.label}
                    className={cn(
                      'flex justify-between py-1.5 border-b border-border/30',
                ...
                {metrics.breakEvenMonths > 0 && cashFlowPositive && (
                  <div className="text-xs text-muted-foreground text-center pt-1">
                    Down payment recoups in ~{metrics.breakEvenMonths} months (
                    {(metrics.breakEvenMonths / 12).toFixed(1)} years)
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <h2 className="font-semibold text-sm mb-4">
                Monthly Rent Growth ({(annualAppreciation * 70).toFixed(0)}% rent appreciation)
              </h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-muted-foreground">
                <div className="p-3 rounded-lg bg-secondary/50">
                  ...
                  <div className="space-y-0.5">
                    <div>
                      <span className="text-emerald-500">≥8%</span> — Great deal
                    </div>
                    <div>
                      <span className="text-amber-500">6-8%</span> — Good
                    </div>
                    <div>
                      <span className="text-red-500">&lt;6%</span> — Weak returns
                    </div>
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-secondary/50">
                  ...
                  <div className="space-y-0.5">
                    <div>
                      <span className="text-emerald-500">≥10%</span> — Excellent
                    </div>
                    <div>
                      <span className="text-amber-500">5-10%</span> — Solid
                    </div>
                    <div>
                      <span className="text-red-500">&lt;5%</span> — Low cash yield
                    </div>
                  </div>
                </div>
                <div className="p-3 rounded-lg bg-secondary/50">
                  ...
                  <div className="space-y-0.5">
                    <div>
                      <span className="text-emerald-500">≤10x</span> — Great buy
                    </div>
                    <div>
                      <span className="text-amber-500">10-15x</span> — OK
                    </div>
                    <div>
                      <span className="text-red-500">&gt;15x</span> — Overpriced
                    </div>
                  </div>
                </div>
              </div>
