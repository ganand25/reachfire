...
export function RothLadderClient(): React.JSX.Element {
  ...
  const setTargetBracket = (v: number): void =>
    setInputs((prev) => ({ ...prev, targetBracket: v }));
  const setFilingStatus = (v: FilingStatus): void =>
    setInputs((prev) => ({ ...prev, filingStatus: v }));
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            The #1 FIRE tax strategy. Convert traditional IRA funds to Roth over 5 years — each
            &quot;rung&quot; becomes penalty-free after the 5-year seasoning period.
          </p>
          <ExportBar
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <StatCard
                ...
                value={plan.totalTaxPaid}
                format="currency"
                subtitle="On conversions"
              />
              <StatCard
                label="Tax Saved"
                value={plan.totalTaxSaved}
                format="currency"
                subtitle="vs no ladder"
                glow
              />
              <StatCard
                label="Bridge Fund Needed"
                value={plan.bridgeFundRequired}
                format="currency"
                subtitle="Taxable accounts"
              />
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <p className="text-xs text-muted-foreground mb-4">
                Each rung converts IRA to Roth. After 5 years of &quot;seasoning,&quot; that rung is
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="border-b border-border">
                      ...
                      <th className="text-left py-2 text-muted-foreground font-medium pl-3">
                        Status
                      </th>
                    </tr>
                  </thead>
                      ...
                      return (
                        <tr key={i} className="border-b border-border/30 hover:bg-accent/20">
                          ...
                          <td className="py-2 text-right tabular-nums font-medium">
                            {formatMoney(rung.conversionAmount)}
                          </td>
                          <td className="py-2 text-right tabular-nums text-amber-400">
                            {formatMoney(rung.taxOnConversion)}
                          </td>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="space-y-2 text-xs text-muted-foreground">
                <div className="flex gap-3">
                  ...
                  <p>
                    Convert $X from Traditional IRA to Roth IRA each year before retiring (pay
                    income tax now)
                  </p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p>
                    Each conversion &quot;seasons&quot; for 5 years — it cannot be withdrawn
                    penalty-free before then
                  </p>
                </div>
                <div className="flex gap-3">
                  <span className="w-5 h-5 rounded-full bg-primary/20 text-primary text-xs flex items-center justify-center shrink-0 font-bold">
                  ...
                  <p>
                    Meanwhile, live off your bridge fund (taxable accounts, cash) during the
                    seasoning period
                  </p>
                </div>
                <div className="flex gap-3">
                  ...
                  <p>
                    After year 5, withdraw from the first rung penalty-free — and repeat each year
                    thereafter
                  </p>
                </div>
              </div>
