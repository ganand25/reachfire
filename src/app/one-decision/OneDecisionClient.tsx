...
} from '@/lib/calculations/decisions';
...
export function OneDecisionClient(): React.JSX.Element {
  ...
  const setCurrentMonthlySavings = (v: number): void =>
    setInputs((prev) => ({ ...prev, currentMonthlySavings: v }));
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground max-w-2xl">
            $5/day feels like nothing. But it&apos;s $56,000 in 20 years at 7% returns — and 8
            months of your life back. See the insane compound impact of a single lifestyle change.
          </p>
          <ExportBar
            ...
            {selectedDecisionObjects.length > 0 && (
              <div className="rounded-xl border border-ember bg-card p-5 glow-ember-sm space-y-3">
                <h2 className="font-semibold text-sm gradient-ember-text">
                  {selectedDecisionObjects.length === 1
                    ? selectedDecisionObjects[0].label
                    : `${selectedDecisionObjects.length} decisions combined`}
                </h2>
                <div className="text-2xl font-bold tabular-nums">
                  {formatMoney(combined.year20Total)}
                </div>
                ...
                <div className="text-xs text-muted-foreground">
                  +${combined.totalMonthlySavings.toLocaleString()}/mo → $
                  {(combined.totalMonthlySavings * 12).toLocaleString()}/yr
                </div>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            {chartData.length > 0 && (
              <div className="rounded-xl border border-border bg-card p-5">
                <h2 className="font-semibold text-sm mb-4">
                  Compound Growth of Selected Decision(s)
                </h2>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground mb-3">
                Click to select one or more decisions and see the combined impact
              </p>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span
                          className={cn(
                            'text-xs font-medium',
                            CATEGORY_COLORS[decision.category] ?? 'text-muted-foreground'
                          )}
                        >
