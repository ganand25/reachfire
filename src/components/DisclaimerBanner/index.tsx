...
const CONTEXT_MESSAGES: Record<string, string> = {
  ...
  'social security':
    'Social Security estimates are approximations based on the SSA bend-point formula. Actual benefits depend on your complete earnings record. Verify at SSA.gov.',
  withdrawal:
    'Withdrawal projections use historical returns and mathematical models. They do not guarantee future portfolio survival. Sequence-of-returns risk is real.',
  healthcare:
    'Healthcare cost estimates vary significantly by state, plan, health status, and future policy changes. Verify current premiums at HealthCare.gov or Medicare.gov.',
  ...
  default:
    'Projections are hypothetical and based on historical data. Past performance does not guarantee future results.',
};
...
export function DisclaimerBanner({
  ...
  return (
    <div className="flex items-start gap-2.5 rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-xs text-muted-foreground">
      ...
      <p>
        <strong className="text-foreground">Not financial advice.</strong> {message}{' '}
