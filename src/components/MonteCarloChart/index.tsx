    ...
    <div className={cn("w-full", className)}>
      ...
      <ResponsiveContainer width="100%" height={200}>
        <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
          ...
          <Area type="monotone" dataKey="p50" stroke="oklch(0.70 0.24 50)" strokeWidth={2} fill="url(#mc50)" />
          <Area type="monotone" dataKey="p25" stroke="none" fill="transparent" />
          <Area type="monotone" dataKey="p10" stroke="oklch(0.52 0.18 250)" strokeWidth={1} strokeDasharray="3 2" fill="transparent" />
        </AreaChart>
      </ResponsiveContainer>
      ...
      <div className="flex gap-3 mt-2 flex-wrap">
        {[
          ...
          <div key={item.label} className="flex items-center gap-1.5 text-xs text-muted-foreground">
            <span className="w-3 h-0.5 inline-block rounded" style={{ backgroundColor: item.color }} />
            {item.label}
          </div>
        ))}
      </div>
